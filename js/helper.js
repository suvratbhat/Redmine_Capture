(function() {
    ...
    this.build = {
        ...
        circle: function() {
            return SVG(paper[0]).size(160, 160).circle(100);
        },

        ellipse: function() {
            return SVG(paper[0]).size(160, 160).ellipse(100, 50);
        },

        line: function() {
            return SVG(paper[0]).size(160, 160).line(0,100,100,0).attr({ fill: 'none', stroke: '#000', 'stroke-width': 4 });
        },

        polyline: function() {
            return SVG(paper[0]).size(160, 160).polyline(polyString).attr({ fill: 'none', stroke: '#000', 'stroke-width': 4 });
        },

        polygon: function() {
            return SVG(paper[0]).size(160, 160).polygon(polyString);
        },

        path: function() {
            return SVG(paper[0]).size(160, 160).path(pathString);
        },

        image: function() {
            return SVG(paper[0]).size(160, 160).image('images/shade.jpg', 120);
        },

        text: function() {
            return SVG(paper[0]).size(160, 160).text('svg.js\nftw').font({ family: 'Source Sans Pro', size: 36 });
        }

    };

    this.describe = function(subject, result) {
        list = $('<ul />');

        $('#results').append($('<h2 />').text(subject)).append(list);

        result();
    };

    this.it = function(subject, result) {
        item  = $('<li />').append($('<h3 />').text(subject)),
            paper = $('<div />').addClass('paper');

        list.append(item.append(paper));
